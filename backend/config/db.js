...
async function initSqliteDb() {
  try {
    ...
    runSql(`CREATE TABLE IF NOT EXISTS Badges (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        description TEXT NOT NULL,
        icon TEXT NOT NULL
    ...
    runSql(`CREATE TABLE IF NOT EXISTS UserBadges (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    runSql(`CREATE TABLE IF NOT EXISTS Posts (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL,
        description TEXT NOT NULL,
    ...
    runSql(`CREATE TABLE IF NOT EXISTS Comments (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        content TEXT NOT NULL,
        userId INTEGER NOT NULL,
    ...
    runSql(`CREATE TABLE IF NOT EXISTS Likes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        userId INTEGER NOT NULL,
        postId INTEGER NOT NULL,
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    runSql(`CREATE TABLE IF NOT EXISTS Notifications (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        userId INTEGER NOT NULL,
        title TEXT NOT NULL,
        message TEXT NOT NULL,
    ...
    runSql(`CREATE TABLE IF NOT EXISTS CommunityAwards (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL,
        month TEXT NOT NULL,
        description TEXT NOT NULL,
    ...
    runSql(`CREATE TABLE IF NOT EXISTS UserCapabilityProfiles (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        userId INTEGER NOT NULL UNIQUE,
        skills TEXT,
        communityExperience TEXT DEFAULT 'Beginner',
        ...
        baseLatitude REAL,
        baseLongitude REAL,
        locationName TEXT DEFAULT 'Việt Nam',
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (userId) REFERENCES Users(id) ON DELETE CASCADE
    )`);

    runSql(`CREATE TABLE IF NOT EXISTS SupportRequests (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        preferredTimeSlot TEXT DEFAULT 'flexible',
        urgencyLevel TEXT DEFAULT 'medium',
        status TEXT DEFAULT 'open',
        matchedVolunteerId INTEGER,
        topMatchScore REAL,
        createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (requesterUserId) REFERENCES Users(id) ON DELETE CASCADE,
    ...
    // Migration an toàn
    try {
      runSql("ALTER TABLE Users ADD COLUMN avatar TEXT");
    } catch(e) {}
    try {
      runSql("ALTER TABLE Posts ADD COLUMN pointsAwarded INTEGER DEFAULT 0");
    } catch(e) {}

    // Check count
    const countRow = sqliteDb.prepare("SELECT COUNT(*) as cnt FROM Users").get();
    const userCount = countRow ? countRow.cnt : 0;
    ...
    if (userCount === 0) {
      ...
      for (const b of badges) {
        runInsert(`INSERT INTO Badges (name, description, icon) VALUES (?, ?, ?)`, b);
      }
      ...
      for (const u of users) {
        runInsert(`INSERT INTO Users (fullName, email, password, avatar, points, level, role) VALUES (?, ?, ?, ?, ?, ?, ?)`, u);
      }
      ...
      for (const ub of userBadges) {
        runInsert(`INSERT INTO UserBadges (userId, badgeId) VALUES (?, ?)`, ub);
      }
      ...
      for (const p of posts) {
        runInsert(`INSERT INTO Posts (title, description, imageUrl, category, latitude, longitude, locationName, status, isFeatured, pointsAwarded, userId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)`, p);
      }
