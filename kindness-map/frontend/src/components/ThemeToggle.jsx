  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <div className={`flex items-center justify-center w-9 h-9 rounded-xl ${isDark ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-100'}`}>
          {isDark ? <SunMedium className="w-4 h-4" /> : <MoonStar className="w-4 h-4" />}
        </div>
        <div className="flex flex-col items-start text-left leading-none">
          <span className="text-xs font-black">{isDark ? 'Light mode' : 'Dark mode'}</span>
