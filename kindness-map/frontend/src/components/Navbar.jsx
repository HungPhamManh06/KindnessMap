              ...
              {demoDropdownOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 py-3 z-50 animate-fade-in overflow-hidden">
                  <div className="px-5 py-3 bg-slate-50 dark:bg-slate-800/50 mb-2">
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Chế độ Demo</p>
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-1 leading-snug">Chuyển vai trò trải nghiệm nhanh</p>
                  </div>
                  <div className="px-2 space-y-1">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200">1. Người Dùng</span>
                      </div>
                      <span className="text-[9px] px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full font-black">520 pts</span>
                    </button>
                    <button
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200">2. Admin</span>
                      </div>
                      <span className="text-[9px] px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full font-black">Quản trị</span>
                    </button>
                  </div>
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2.5 ml-1">
                ...
                <div className="relative" ref={notifRef}>
                  ...
                  {notifDropdownOpen && (
                    <div className="absolute right-0 mt-3 w-80 sm:w-96 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 py-4 z-50 animate-fade-in divide-y divide-slate-100 dark:divide-slate-800 max-h-[480px] flex flex-col overflow-hidden">
                      <div className="px-6 py-2 flex items-center justify-between shrink-0 mb-2">
                        <h4 className="font-black text-sm text-slate-800 dark:text-slate-100 uppercase tracking-tight">Thông báo</h4>
                        {unreadCount > 0 && (
                          <button
                      ...
                      <div className="overflow-y-auto flex-1 custom-scrollbar">
                        {notifications.length === 0 ? (
                          <div className="p-10 text-center">
                            ...
                            <p className="text-xs font-bold text-slate-400">Bạn chưa có thông báo mới nào</p>
                          </div>
                        ) : (
                          notifications.map((notif) => (
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                <span className={`text-xs font-bold ${notif.isRead ? 'text-slate-600 dark:text-slate-400' : 'text-slate-900 dark:text-white'}`}>{notif.title}</span>
                ...
                <div className="relative" ref={profileRef}>
                  ...
                  >
                    <div className="relative">
                      <img src={user.avatar} alt={user.fullName} className="w-10 h-10 rounded-xl object-cover ring-2 ring-white dark:ring-slate-900 shadow-sm" />
                  ...
                  {profileDropdownOpen && (
                    <div className="absolute right-0 mt-3 w-72 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 p-2 z-50 animate-fade-in overflow-hidden">
                      <div className="px-4 py-4 bg-slate-50 dark:bg-slate-800/50 rounded-3xl mb-2 text-center">
                        <img src={user.avatar} className="w-16 h-16 rounded-2xl mx-auto mb-3 object-cover shadow-lg" alt="" />
                        <p className="font-black text-sm text-slate-900 dark:text-white">{user.fullName}</p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">{user.email}</p>
